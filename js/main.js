    ...
    if ($('.searchbar').length) {
        $(".searchbar").autocomplete({
            source: function (request, response) {
                $.getJSON("/php/autocomplete.php", { term: $('.searchbar').val(), display: $('#autoValues').val() },
                    response);
            },
...
$(function () {
    if ($('.small-search').length) {
        $(".small-search").autocomplete({
            source: function (request, response) {
                $.getJSON("/php/autocomplete.php", { term: $('#small-search-auto').val(), display: $('#autoValues1').val() },
                    response);
            },
    ...
    if ($('.field-autocomplete').length) {
        $(".field-autocomplete").autocomplete({
            ...
            focus: function (event, ui) {
                event.preventDefault();
                jQuery(this).val(ui.item[6]);
                $(this).parent().children('.autoValues').val(ui.item[8]);
                return false;
            },
            messages: {
                noResults: '',
        ...
        initValidation: function () {
            ...
            $(".resource-form-validate").validate({
                ...
                messages: {
                    ...
                    url: {
                        url: 'Please enter a valid URL (ex. http://scicrunch.com)'
                    },
                    Resource_URL: {
                        url: 'Please enter a valid URL (ex. http://scicrunch.com)'
                    }
